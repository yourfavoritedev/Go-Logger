...
import (
	...
	"github.com/yourfavoritedev/proglog/internal/agent"
...
// setupConfig reads the configration and returns the agent's configuration. Cobra calls setupConfig before running the command's runE function.
func (c *cli) setupConfig(cmd *cobra.Command, args []string) error {
	...
	if err = viper.ReadInConfig(); err != nil {
		// it's ok if config file doesn't exist (asserts that this an error, if not error type, then return err set by viper.ReadInConfig)
		if _, ok := err.(viper.ConfigFileNotFoundError); !ok {
			return err
		}
	}

	// setup config from key-value store
...
// run runs the executable logic by,
// creating the agent, handling signals from the operating system and,
// shutting down the agent gracefully wehen the operating system terminates the program
func (c *cli) run(cmd *cobra.Command, args []string) error {
	...
	// need a buffered channel to be used by signal.Notify
